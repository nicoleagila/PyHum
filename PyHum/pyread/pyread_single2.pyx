       ...
       if 2>1:
          ...
          try: #faster to use the idx file, if it exists
             ...
             while result is None:
               try:
                 spacer = self._fread(idxfid, 4, 'B') #guarda 4 bytes del archivo .IDX en un buffer de unsigned char
                 dfbreak.append(struct.unpack('>i', ''.join(self._fread(idxfid,4,'c')) )[0]) #desempaca el buffer de string en formato big-endian int
               except:
                 result = 1
             idxfid.close()
             dfbreak = np.diff(dfbreak).tolist() #diferencia finita

             fid = open(sonfile,'rb')
             for i from 0 <= i < len(dfbreak):       
                tmpdata.append(self._gethead(fid,trans,c, model, humdat['linesize'])) #extrae los bits del head
                ints_list = []
                for j from 0 <= j < dfbreak[i]-headbytes:                 
                   ints_list.append(struct.unpack('>B', ''.join(self._fread(fid,1,'c')) )[0]) #extrae los datos del sonar
...
#GUARDAR IMAGEN
        im = Image.fromarray(np.asarray(c_scan))
        im.save("{}-raw.tiff".format(sonarstring))
